...
struct elementC {
    vector<vector<double>> N;  // Macierz funkcji kształtu N[4][npc]
    vector<vector<double>> Cpc;  // Macierz Cpc
};

//struct Elem2d_4 {
//    double dNdKsi[16][4];  // Pochodne funkcji kształtu względem ksi
//    double dNdEta[16][4];  // Pochodne funkcji kształtu względem eta
//};

 struct Surface {
    vector<vector<double>> N; // Funkcje kształtu dla punktów Gaussa
};

struct SurfaceElement {
    vector<Surface> surfaces;

    SurfaceElement() : surfaces(4) {}
};
...
void LoadData(const string& filename, Node& node, Element& element, Grid& grid, GlobalData& globalData) {
    ...
    for (current_line = 0; current_line < grid.nE; current_line++) {
        ...
        grid.El[current_line].ID[0] = static_cast<int>(number);
        file >> number >> word;
...
void calculateDN_dx_dy(const vector<vector<double>>& dN_dKsi, const vector<vector<double>>& dN_dEta, Jakobian& jacobian, int npc) {
    // Initialize inverse Jacobian matrix for each point
    for (int pc = 0; pc < npc; ++pc) {
...
void calculate_H_matrices(const Jakobian& jacobian, H& h, int npc) {
    ...
    for (int pc = 0; pc < npc; ++pc) {
        ...
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                h.Hpcx[i][j][pc] = jacobian.dN_dX[pc][i] * jacobian.dN_dX[pc][j];
...
void calculate_Hpc_matrix(H& h, double k, const Jakobian& jacobian, int npc) {
    for (int pc = 0; pc < npc; ++pc) {
        ...
        // Wyświetlenie wyników macierzy Hpc dla danego punktu Gaussa
...
void calculateH(H& h, const vector<double>& weights, const int npc, int elementIndex, int N, Grid& grid, vector<vector<double>>& H_globalna) {
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            ...
            H_result[0][0] += Hpc[0][0];
            H_result[0][1] += Hpc[0][1];
            H_result[0][2] += Hpc[0][2];
            H_result[0][3] += Hpc[0][3];

            H_result[1][0] += Hpc[1][0];
            H_result[1][1] += Hpc[1][1];
            H_result[1][2] += Hpc[1][2];
            H_result[1][3] += Hpc[1][3];

            H_result[2][0] += Hpc[2][0];
            H_result[2][1] += Hpc[2][1];
            H_result[2][2] += Hpc[2][2];
            H_result[2][3] += Hpc[2][3];

            H_result[3][0] += Hpc[3][0];
            H_result[3][1] += Hpc[3][1];
            H_result[3][2] += Hpc[3][2];
            H_result[3][3] += Hpc[3][3];
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            int node_i = elementNodes[i] - 1;   // Indeks węzła i
...
void printShapeFunctions(const SurfaceElement& surfaceElement, int gaussOrder) {
    for (int edge = 0; edge < 4; edge++) {
        cout << "Edge " << edge + 1 << ":\n";
        for (int i = 0; i < gaussOrder; i++) {
            cout << "  Gauss Point " << i + 1 << ": ";
            for (int j = 0; j < 4; j++) {
                cout << "N" << j + 1 << " = " << fixed << setprecision(4)
                    << surfaceElement.surfaces[edge].N[i][j] << "  ";
            }
            cout << "\n";
        }
        cout << "\n";
...
void calculateC(H& h, const int npc, int N, Grid& grid, vector<vector<double>>& C_globalna, const Jakobian& jacobian, double c, double rho, const elementC& elC, Element& element) {
    ...
    vector<vector<vector<double>>> Cpc_per_point(npc, vector<vector<double>>(4, vector<double>(4, 0.0)));
    ...
    for (int row = 0; row < 4; ++row) {
        for (int col = 0; col < 4; ++col) {
            element.C_local[row][col] = 0.0;  // Zerowanie macierzy przed sumowaniem
        }
    }
    ...
    for (int pc_x = 0; pc_x < N; ++pc_x) {
        for (int pc_y = 0; pc_y < N; ++pc_y) {
            double detJ = jacobian.detJ[pc_x * N + pc_y];  // Wyznacznik Jacobiana dla punktu Gaussa

            // Obliczanie Cpc dla punktu (pc_x, pc_y)
            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < 4; ++j) {
                    // Wagi Gaussa w dwóch wymiarach (ksi i eta)
                    double weight_x = gaussWeights[pc_x];
                    double weight_y = gaussWeights[pc_y];

                    // Obliczanie Cpc = c * rho * (N_i * N_j) * detJ * w_x * w_y dla danego punktu
                    Cpc_per_point[pc_x * N + pc_y][i][j] = c * rho * (elC.N[pc_x * N + pc_y][i] * elC.N[pc_x * N + pc_y][j]) * detJ * weight_x * weight_y;
                }
            }
            ...
            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < 4; ++j) {
                    element.C_local[i][j] += Cpc_per_point[pc_x * N + pc_y][i][j];
                }
            }
        }
    }

    // Wypisanie końcowej macierzy C_local
    //cout << "Lokalna macierz C (C_local):" << endl;
    //for (int row = 0; row < 4; ++row) {
    //    for (int col = 0; col < 4; ++col) {
    //        // Wypisanie wartości C z określoną precyzją
    //        cout << fixed << setprecision(4) << element.C_local[row][col] << " ";
    //    }
    //    cout << endl;
    //}
    ...
    // Sumowanie lokalnych wartości C_local do C_globalna
    for (int i = 0; i < 4; ++i) {  // Przechodzimy po wierszach lokalnej macierzy C
        for (int j = 0; j < 4; ++j) {  // Przechodzimy po kolumnach lokalnej macierzy C
            // Indeksowanie globalne węzłów
            int node_i = elementNodes[i] - 1;  // Indeks globalny węzła i (pomniejszamy o 1)
            int node_j = elementNodes[j] - 1;  // Indeks globalny węzła j (pomniejszamy o 1)

            // Dodanie wartości z lokalnej macierzy C do globalnej
            C_globalna[node_i][node_j] += element.C_local[i][j];  // Indeksowanie w macierzy globalnej 16x16
        }
    }
}
...
void calculateHbc(Element& element, vector<Node>& nodes, double alfa, int gaussOrder, H& h, double t_otoczenia, int elementIndex, vector<vector<double>>& H_globalna, vector<double>& P_global) {
    ...
    for (int edge = 0; edge < 4; edge++) {
        int n1 = element.ID[edge % 4] - 1;
        int n2 = element.ID[(edge + 1) % 4] - 1;

        if (nodes[n1].BC == 1 && nodes[n2].BC == 1) {
...
vector<double> gaussianElimination(vector<vector<double>>& H_globalna, vector<double>& P_global) {
    ...
    // Eliminacja Gaussa
    for (int k = 0; k < n; ++k) {
        // Szukanie maksymalnego elementu w kolumnie k (dla stabilności numerycznej)
        int maxIndex = k;
        for (int i = k + 1; i < n; ++i) {
...
void gaussElimination(vector<vector<double>>& A, vector<double>& b, vector<double>& x) {
    ...
    // Przeprowadzanie eliminacji Gaussa
    for (int i = 0; i < n; ++i) {
        // Szukanie największego elementu w kolumnie i
    ...
    // Rozwiązywanie układu równań po eliminacji
    x.resize(n);
    for (int i = n - 1; i >= 0; --i) {
        x[i] = b[i] / A[i][i];
        for (int j = i - 1; j >= 0; --j) {
...
int main() {
   ...
   // Elem2d_4 elem2d_4;
    ...
    LoadData("Test3_31_31_kwadrat.txt", node, element, grid, globalData); // Test2_4_4_MixGrid.txt , Test1_4_4.txt , test.txt, Test3_31_31_kwadrat.txt
    ...
    double c = globalData.density;
    double rho = globalData.specificHeat;
    ...
    if (N == 2) {
        ...
        // Create the 2x2 grid of integration points
    ...
    for (int step = 0; step < numTimeSteps; ++step) {
      ...
      /*  cout << "Macierz [H] + [C]/dT dla kroku czasowego " << step + 1 << ":" << endl;
       ...
       /* cout << "Wektor temperatury po kroku czasowym " << step + 1 << ":" << endl;
        ...
        double minTemp = *min_element(x.begin(), x.end());
        double maxTemp = *max_element(x.begin(), x.end());
